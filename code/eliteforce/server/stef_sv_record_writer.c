...
#include "stef_sv_record_local.h"
#include <time.h>
...
static void Record_UpdateEntityset( record_entityset_t *entities ) {
	...
	if ( sv_recordVerifyData->integer ) {
		...
		verify_entities = (record_entityset_t *)Z_Malloc( sizeof( *verify_entities ) );
	...
	if ( sv_recordVerifyData->integer ) {
		Record_DecodeEntityset( verify_entities, &verify_stream );
		...
		} else if ( Record_CompareEntityStates( entities, verify_entities, qtrue ) ) {
...
static void Record_UpdatePlayerstate( playerState_t *ps, int clientNum ) {
	...
	if ( sv_recordVerifyData->integer ) {
		Record_DecodePlayerstate( &verify_ps, &verify_stream );
		if ( verify_stream.position != rws->stream.position ) {
...
static void Record_UpdateVisibilityState( record_visibility_state_t *vs, int clientNum ) {
	...
	if ( sv_recordVerifyData->integer ) {
		Record_DecodeVisibilityState( &verify_vs, &verify_stream );
		if ( verify_stream.position != rws->stream.position ) {
...
#ifdef ELITEFORCE
	record_usercmd_t recordUsercmd;
	record_data_stream_t verifyStream;
	record_usercmd_t verifyUsercmd;

	Record_RepackUsercmd( usercmd, &recordUsercmd );
#else
	record_data_stream_t verifyStream;
...
#ifdef ELITEFORCE
	Record_EncodeUsercmd( &rws->rs->clients[clientNum].usercmd, &recordUsercmd, &rws->stream );
#else
...
#ifdef ELITEFORCE
		} else if ( memcmp( &recordUsercmd, &verifyUsercmd, sizeof( recordUsercmd ) ) ) {
#else
...
static void Record_StartWriter( int maxClients, qboolean autoStarted ) {
	...
	// Write the protocol
#ifdef ELITEFORCE
	Record_Stream_WriteValue( RECORD_PROTOCOL, 4, &rws->stream );
#else
...
void Record_Writer_ProcessUsercmd( usercmd_t *usercmd, int clientNum ) {
	...
	if ( !sv_recordFullUsercmdData->integer ) {
		...
		Record_UnpackUsercmd( &rws->rs->clients[clientNum].usercmd, &oldUsercmd );
