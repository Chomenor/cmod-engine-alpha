...
static pure_checksum_entry_t *FS_GetPureChecksumEntry( const fsc_file_direct_t *pk3 ) {
	...
	FSC_LoadPk3( (fsc_ospath_t *)STACKPTR( pk3->os_path_ptr ), &fs.index, FSC_SPNULL, FS_GetPureChecksumEntryCallback, &entry );
	if ( entry ) {
		entry->pk3 = pk3;
	}
	return entry;
...
const char *FS_ReferencedPakPureChecksums( int maxlen ) {
	...
	FS_BuildPureValidationString( buffer, maxlen, &reference_tracker );
	return buffer;
}
...
static void FS_RefSet_InsertEntry( reference_set_work_t *rsw, const char *mod_dir, const char *name,
	...
	if ( rsw->query->download ) {
		...
		if ( !Q_stricmp( mod_dir, fs.cvar.fs_basegame->string ) ) {
			mod_dir = fs.cvar.fs_basegame->string;
		}
...
static void FS_RefSet_ProcessManifest( reference_set_work_t *rsw, const char *string, int recursion_count ) {
	while ( 1 ) {
		const char *token = COM_ParseExt( (const char **)&string, qfalse );
		if ( !*token ) {
			break;
		}
		...
		if ( !Q_stricmp( token, "&cvar_import" ) ) {
			...
			char cvar_name[256];
			token = COM_ParseExt( (const char **)&string, qfalse );
...
void FS_GenerateReferenceLists( void ) {
	...
	if ( Cvar_VariableIntegerValue( "sv_pure" ) ) {
		int systeminfo_base_length = FSC_Strlen( Cvar_InfoString_Big( CVAR_SYSTEMINFO, NULL ) );
