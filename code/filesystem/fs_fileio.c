...
#define fs_handle_owner_t handleOwner_t
#define FS_HANDLEOWNER_SYSTEM H_SYSTEM
#define FS_HANDLEOWNER_CGAME H_CGAME
#define FS_HANDLEOWNER_UI H_Q3UI
#define FS_HANDLEOWNER_QAGAME H_QAGAME
...
NOTE: This version does not perform any path sanitization. Ensure it is only called with safe path inputs.
===========
*/
void FS_Remove( const char *osPath ) {
	FSC_DeleteFile( osPath );
}
...
void FS_Rename( const char *from, const char *to ) {
	char from_ospath[FS_MAX_PATH];
	char to_ospath[FS_MAX_PATH];
	fsc_filehandle_t *f;

	if ( !FS_GeneratePathWritedir( FS_GetCurrentGameDir(), from, 0, FS_ALLOW_DIRECTORIES, from_ospath, sizeof( from_ospath ) ) ||
			!FS_GeneratePathWritedir( FS_GetCurrentGameDir(), to, FS_CREATE_DIRECTORIES,
			FS_ALLOW_DIRECTORIES | FS_CREATE_DIRECTORIES_FOR_FILE, to_ospath, sizeof( to_ospath ) ) ) {
		Com_Printf( "FS_Rename: Error generating paths: from(%s) to(%s)\n", from, to );
		return;
	}

	if ( fs.cvar.fs_debug_fileio->integer ) {
		FS_DPrintf( "********** rename file **********\n" );
		FS_DPrintf( "  source: %s\n", from_ospath );
		FS_DPrintf( "  destination: %s\n", to_ospath );
	}

	f = FSC_FOpen( from_ospath, "rb" );
	if ( !f ) {
		FS_DPrintf( "  Rename failed - unable to open source file\n" );
		return;
	}
	FSC_FClose( f );
	FS_Remove( to_ospath );
	
	FSC_RenameFile( from_ospath, to_ospath );

	/*
	if ( FSC_RenameFile( from_ospath, to_ospath ) ) {
		// Failed, try copying it and deleting the original
		FS_DPrintf( "Standard rename failed - attempting copy/delete method\n" );
...
char *FS_ReadData( const fsc_file_t *file, const char *path, unsigned int *size_out, const char *calling_function ) {
	...
	fsc_filehandle_t *fsc_file_handle = NULL;
	unsigned int size;
	...
	if ( path ) {
		...
		os_path = (fsc_ospath_t *)STACKPTR( ( (fsc_file_direct_t *)file )->os_path_ptr );
	}
...
=================
*/
fileHandle_t FS_CacheReadHandle_Open( const fsc_file_t *file, const char *path, unsigned int *size_out ) {
	char buffer[FS_FILE_BUFFER_SIZE];
...
#ifdef _WIN32
		osHandle = _popen( cmd, "wb" );
#else
		osHandle = popen( cmd, "w" );
#endif
	}

	if ( !osHandle ) {
		Com_Printf( "Failed to open pipe file: %s\n", path );
...
fileHandle_t FS_OpenSettingsFileWrite( const char *filename ) {
	...
	if ( fs.cvar.fs_mod_settings->integer ) {
		...
		mod_dir = fs.cvar.fs_basegame->string;
	}
...
In some cases, mods may attempt to read files that were just created by the mod/engine.
...
long FS_SV_FOpenFileRead( const char *filename, fileHandle_t *fp ) {
	...
	*fp = FS_INVALID_HANDLE;
	...
	for ( i = 0; i < FS_MAX_SOURCEDIRS; ++i ) {
		if ( FS_GeneratePathSourcedir( i, filename, NULL, FS_ALLOW_DIRECTORIES | FS_ALLOW_PK3, 0, path, sizeof( path ) ) ) {
...
int FS_Home_FOpenFileRead( const char *filename, fileHandle_t *fp ) {
	...
	*fp = FS_INVALID_HANDLE;
	...
	if ( fs.cvar.fs_debug_fileio->integer ) {
		FS_DPrintf( "********** Home file read **********\n" );
		FS_DebugIndentStart();
		FS_DPrintf( "path: %s\n", filename );
	}

	if ( FS_GeneratePathWritedir( FS_GetCurrentGameDir(), filename, 0, FS_ALLOW_DIRECTORIES, path, sizeof( path ) ) ) {
		*fp = FS_DirectReadHandle_Open( NULL, path, (unsigned int *)&size );
	}
...
int FS_Seek( fileHandle_t f, long offset, fsOrigin_t origin ) {
	return FS_Handle_FSeek( f, offset, origin );
}
...
int FS_VM_OpenFile( const char *qpath, fileHandle_t *f, fsMode_t mode, handleOwner_t owner ) {
	return FS_FOpenFileByModeGeneral( qpath, f, mode, owner );
}
...
void FS_SV_Rename( const char *from, const char *to ) {
	...
	if ( !FS_GeneratePathWritedir( from, NULL, FS_ALLOW_DIRECTORIES, 0, source_path, sizeof( source_path ) ) ) {
		Com_Printf( "FS_SV_Rename: Invalid source path '%s'\n", from );
		return;
	}
	if ( !FS_GeneratePathWritedir( to, NULL, FS_ALLOW_DIRECTORIES | FS_ALLOW_PK3 | FS_CREATE_DIRECTORIES_FOR_FILE, 0,
			target_path, sizeof( target_path ) ) ) {
		Com_Printf( "FS_SV_Rename: Invalid target path '%s'\n", to );
		return;
	}
...
qboolean FS_SV_FileExists( const char *file ) {
	...
	for ( i = 0; i < FS_MAX_SOURCEDIRS; ++i ) {
		if ( FS_GeneratePathSourcedir( i, file, NULL, FS_ALLOW_DIRECTORIES, 0, path, sizeof( path ) ) ) {
