...
static qboolean FS_FileList_CheckPathListable( fsc_stream_t *stream, const filelist_work_t *flw ) {
	...
	if ( fs.fnamecallback ) {
		if ( !fs.fnamecallback( stream->data, stream->position ) ) {
			return qfalse;
		}
	}
	else if ( flw->extension ) {
		if ( !FS_FileList_PatternMatch( stream->data, flw->extension, qtrue ) ) {
			return qfalse;
		}
	}
	if ( flw->filter ) {
		if ( !Com_FilterPath( (char *)flw->filter, stream->data ) ) {
			return qfalse;
		}
	}
	return qtrue;
...
static qboolean FS_FileList_CheckFileListable( const fsc_file_t *file, const filelist_work_t *flw ) {
	...
	if ( ( flw->flags & LISTFLAG_DIRECT_SOURCE_ONLY ) && file->sourcetype != FSC_SOURCETYPE_DIRECT ) {
		return qfalse;
	}
	if ( ( flw->flags & LISTFLAG_PK3_SOURCE_ONLY ) && file->sourcetype != FSC_SOURCETYPE_PK3 ) {
		return qfalse;
	}
...
static int FS_GetModList( char *listbuf, int bufsize ) {
	...
	for ( i = 0; i < list.count; ++i ) {
		...
		if ( !Q_stricmp( mod_name, fs.cvar.fs_basegame->string ) ) {
			continue;
		}
...
char **FS_ListFilteredFiles_Flags( const char *path, const char *extension,
	...
	// Current Quake3e behavior is to list external files even when connected to a pure
	// server in most cases. This does not apply if extension is NULL, but does apply if
	// extension is empty string.
	if ( extension ) {
		query.flags |= LISTFLAG_PURE_ALLOW_DIRECT_SOURCE;
	}
