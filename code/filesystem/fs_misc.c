...
fs_modtype_t FS_GetModType( const char *mod_dir ) {
	if ( mod_dir ) {
		...
		else if ( !Q_stricmp( sanitized_mod_dir, fs.cvar.fs_basegame->string ) )
			return MODTYPE_BASE;
	}
...
static void FS_Servercfg_UpdateState( void ) {
	if ( fs.cvar.fs_servercfg->modificationCount != servercfg_cvar_mod_count ) {
		const char *servercfg_ptr = fs.cvar.fs_servercfg->string;
		const char *token;
...
#ifdef DEBUG
	if ( FS_GeneratePath( Sys_Pwd(), name, NULL, FS_NO_SANITIZE, FS_ALLOW_DLL, 0, path, sizeof( path ) ) ) {
		libHandle = Sys_LoadLibrary( path );
	}
#endif

	for ( i = 0; !libHandle && i < FS_MAX_SOURCEDIRS; ++i ) {
		if ( FS_GeneratePathSourcedir( i, name, NULL, FS_ALLOW_DLL, 0, path, sizeof( path ) ) ) {
			libHandle = Sys_LoadLibrary( path );
		}
	}
...
=================
*/
void *FS_LoadGameDLL( const fsc_file_t *dll_file, vmMainFunc_t *entryPoint, dllSyscall_t systemcalls ) {
	char longName[FS_FILE_BUFFER_SIZE];
	const char *shortName = (const char *)STACKPTR( dll_file->qp_name_ptr );
	const fsc_ospath_t *dll_path;
	...
	FS_FileToBuffer( dll_file, longName, sizeof( longName ), qtrue, qtrue, qtrue, qfalse );
	...
	dll_path = (const fsc_ospath_t *)STACKPTR( ( (fsc_file_direct_t *)dll_file )->os_path_ptr );
	...
	dll_handle = Sys_LoadLibrary( dll_path_string );
	...
	Com_Printf( "FS_LoadGameDLL '%s' ok\n", shortName );

	dllEntry = (dllEntry_t)Sys_LoadFunction( dll_handle, "dllEntry" );
	*entryPoint = (vmMainFunc_t)Sys_LoadFunction( dll_handle, "vmMain" );
	if ( !*entryPoint || !dllEntry ) {
		Sys_UnloadLibrary( dll_handle );
		return NULL;
	}
...
void FS_FilenameCompletion( const char *dir, const char *ext, qboolean stripExt,
	...
	filenames = FS_ListFilteredFiles_Flags( dir, ext, NULL, &nfiles, flags );
...
void FS_CheckCorePaks( void ) {
	...
	if ( Q_stricmp( fs.cvar.fs_basegame->string, BASEGAME ) ) {
